...
// Read existing gifts
const existingContent = fs.readFileSync('src/constants/gifts.ts', 'utf8');

// The 124 new items curated with love, exact URLs and rich Unsplash photos
const newGifts = [
  // 34. Jarra (8)
  {
    id: 'jarra-01',
  ...
  // 58. Kit de utensílios de cozinha (1)
  {
  ...
  // 80. Kit de limpeza / LAVANDERIA (9)
  {
    id: 'limpeza-01',
...
// Check unique IDs in newGifts
const ids = new Set();
newGifts.forEach(g => {
  if (ids.has(g.id)) throw new Error('Duplicate new ID: ' + g.id);
  ids.add(g.id);
});

console.log('Total new gifts to append:', newGifts.length);

// Extract existing GIFTS_DATA from gifts.ts
const giftsDataMatch = existingContent.match(/export const GIFTS_DATA: GiftItem\[\] = (\[[\s\S]*?\]);/);
if (!giftsDataMatch) {
  throw new Error('Could not find GIFTS_DATA in gifts.ts');
}

const existingData = JSON.parse(giftsDataMatch[1]);
console.log('Existing gifts count:', existingData.length);

// Verify no duplicated URLs between existing and new
const existingUrls = new Set(existingData.map(g => g.url));
newGifts.forEach(g => {
  if (existingUrls.has(g.url)) {
    console.warn('Warning: URL already in existing gifts:', g.url);
  }
});

// Combine
const combined = [...existingData, ...newGifts];
console.log('Combined gifts count:', combined.length);

// New categories
const categoriesCode = `export const GIFT_CATEGORIES = [
  "Todos",
  "Copos, Taças & Xícaras",
  "Mesa & Servir",
...
const newContent = `export interface GiftItem {
  id: string;
  title: string;
  category: string;
  description: string;
...
export const GIFTS_DATA: GiftItem[] = ${JSON.stringify(combined, null, 2)};
`;

fs.writeFileSync('src/constants/gifts.ts', newContent, 'utf8');
console.log('Successfully updated src/constants/gifts.ts!');
